import type { Resume, Employer, Vacancy, Invitation, Message, AdminStats, AuditLog, Dictionaries, WorkExperience, ParsedSource, RegionStat, SpecialStatus, ResumeTest } from './types';
...
const _specialStatusValues: SpecialStatus[] = [
  { value: 'svo_participant', label: 'Участник СВО', confirmed: false, docDate: '', docNumber: '', documentRef: '', disabilityGroup: '' },
  { value: 'svo_family', label: 'Член семьи участника СВО', confirmed: false, docDate: '', docNumber: '', documentRef: '', disabilityGroup: '' },
  { value: 'disabled', label: 'Человек с ОВЗ', confirmed: false, docDate: '', docNumber: '', documentRef: '', disabilityGroup: '' },
];
const _testValues: ResumeTest[] = [
  { value: 'test_44fz_customer', label: '44-ФЗ для заказчиков', passedAt: null },
  { value: 'test_223fz_customer', label: '223-ФЗ для заказчиков', passedAt: null },
  { value: 'etp_zakaz_rf', label: 'ЭТП ЗаказРФ', passedAt: null },
];
...
export const RESUMES: Resume[] = Array.from({ length: 75 }, (_, i): Resume => {
  ...
  return {
    ...
    skills: [],
    purchaseTypes: [],
    tests: pick(_testValues, testsCount, i * 17),
    specialStatuses: hasSpecial ? [_specialStatusValues[i % _specialStatusValues.length]] : [],
    hasPhoto,
...
export const DICTIONARIES: Dictionaries = {
  ...
  tests: _testValues.map(t => t.label),
  specialStatuses: _specialStatusValues.map(s => s.label),
